...
import { StorageService } from '../../../../app/modules/auth/api-authenticate/storage.service'; 
...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    )
    {
        ...
        if (currentUserString !== null) {
            ...
            console.error('Nenhum usuário atual encontrado no localStorage.');
        }
    ...
    ngOnInit(): void
    {
        ...
        this._router.events
            ...
            /* configuraões da tela */
            let config = localStorage.getItem('configuracoes');  
            if (config !== null) { 
                
                let parsedConfig = JSON.parse(config); 
                this.GetConfiguracoes(!!parsedConfig.menu);
            }
    ...
    {
        this.GetConfiguracoes(open); 

    }

    GetConfiguracoes(open: boolean) { 
 

            this.opened = open;         // Set the opened 
            this._enableAnimations();   // Enable the animations 

            // If the navigation opened, and the mode - is 'over', show the overlay
            if ( this.mode === 'over' )
            {
                if ( this.opened )
                {
            ...
            this._hideOverlay();
            this.openedChanged.next(open); // Execute the observable
